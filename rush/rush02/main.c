...
/*   Updated: 2024/04/07 22:14:23 by chrrodri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_verify_number(char* filename, long long target)
{
	...
	else if (target < 100)
	{
		...
		hundreds = target / 100;
		remainder = target % 100;
		if (hundreds >= 1) 
			ft_verify_number(filename, hundreds);
	...
	else
	{
		...
		lead = target / place;
		l_remainder = target % place;
		ft_verify_number(filename, lead);
		find_value_in_file(filename, place);
		if (l_remainder > 0)
		{
			if (l_remainder < 100)
				find_value_in_file(filename, 0);
			ft_verify_number(filename, l_remainder);
		}
	}
}
